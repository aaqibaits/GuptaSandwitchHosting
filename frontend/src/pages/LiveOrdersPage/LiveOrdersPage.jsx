...
const LiveOrdersPage = ({ showToast, orders = [], setOrders, stats, isConnected, soundEnabled, setSoundEnabled }) => {
  const [activePage, setActivePage] = useState('orders');
  const [confirmReject, setConfirmReject] = useState({ isOpen: false, orderId: null });
  const [confirmDelete, setConfirmDelete] = useState({ isOpen: false, dishId: null, dishName: '' });
  ...
  const rejectOrder = (id) => {
    setConfirmReject({ isOpen: true, orderId: id });
  };
  ...
  const handleConfirmReject = async () => {
    ...
    try {
      ...
      setConfirmReject({ isOpen: false, orderId: null });
    }
  };
  ...
  const handleConfirmDelete = () => {
    ...
    setConfirmDelete({ isOpen: false, dishId: null, dishName: '' });
  };
  ...
  return (
    <div className="live-orders-page">
      <div className="live-orders-header">
        ...
        <div className="header-controls">
          ...
          <button className="top-btn" onClick={() => setAutoAccept(v => !v)}>
            <i className={`ti ${autoAccept ? 'ti-checkbox' : 'ti-square'}`}></i>
            Auto-Accept
          </button>
        </div>
      </div>
                      ...
                      <tr key={dish.id} className={!isAvailableOnCurrentPlatform ? 'disabled-row' : ''}>
                        ...
                        <td className="th-center">
                          <div className="row-actions">
                            ...
                            <button className="row-btn row-btn--delete" onClick={() => setConfirmDelete({ isOpen: true, dishId: dish.id, dishName: dish.name })} title="Delete"><i className="ti ti-trash"></i></button>
                          </div>
                        </td>
                      </tr>
                    );
      ...
      <ConfirmationModal
        ...
        onCancel={() => setConfirmReject({ isOpen: false, orderId: null })}
      />
      <ConfirmationModal
        isOpen={confirmDelete.isOpen}
        title="Delete Dish"
        message={`Are you sure you want to delete "${confirmDelete.dishName}"?`}
        isDanger={true}
        confirmText="Delete"
        onConfirm={handleConfirmDelete}
        onCancel={() => setConfirmDelete({ isOpen: false, dishId: null, dishName: '' })}
      />
    </div>
  );
};
